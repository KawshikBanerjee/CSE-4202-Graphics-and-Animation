// Q2. Show an OpenGL Program which will show three different triangles with red, black and blue color in   green background.
// roll: 1703016
...
int main()
{
    ...
    unsigned int fragmentShaderBlack = glCreateShader(GL_FRAGMENT_SHADER); // the second fragment shader that outputs the color black
    unsigned int fragmentShaderBlue = glCreateShader(GL_FRAGMENT_SHADER); // black
    
    unsigned int shaderProgramRed = glCreateProgram();
    unsigned int shaderProgramBlack = glCreateProgram(); // the second shader program
    unsigned int shaderProgramBlue = glCreateProgram(); // third
